...
class MockDatabase {
  ...
  // No-op for compatibility with App.tsx
  ...
  async login(email: string, password: string): Promise<User | null> {
    ...
    if (cleanEmail === INITIAL_ADMIN.email && cleanPass === '123456') return INITIAL_ADMIN;
    
    const agent = this.agents.find(a => a.email === cleanEmail);
    if (agent && agent.active && agent.password === cleanPass) {
